...
$token = \Widget\Options::alloc()->plugin('AISummary')->token;
$url = '/action/ai-summary?do=generate&token=' . $token;
?>
<div class="main">
                        ...
                        <div class="operate">
                            ...
                            <div class="btn-group btn-drop">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a href="<?php $options->index($url); ?>"><?php _e('生成摘要'); ?></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <table class="typecho-list-table">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th><?php _e('标题'); ?></th>
                                    <th><?php _e('摘要'); ?></th>
                                    <th class="kit-hidden-mb"><?php _e('字数'); ?></th>
                                    <th><?php _e('操作'); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <tr id="<?php $posts->theId(); ?>">
                                            ...
                                            $row = $db->fetchRow(
                                                $db->select('str_value')->from('table.fields')
                                                    ->where('cid = ? AND name = ?', $posts->cid, 'customSummary')
                                            );
                                            if ($row && !empty($row['str_value'])) {
                                                $summaryText = htmlspecialchars($row['str_value']);
                                                $summaryLen  = mb_strlen($row['str_value'], 'UTF-8');
                                                echo '<td>' . $summaryText . '</td>';
                                                echo '<td class="kit-hidden-mb">' . $summaryLen . '</td>';
                                            } else {
                                                echo '<td><span style="color:#999">未生成</span></td>';
                                                echo '<td class="kit-hidden-mb">-</td>';
                                            }
                                            ?>
                                            <td><a href="<?php $options->index($url . '&cid=' . $posts->cid); ?>"><?php _e('生成'); ?></a></td>
                                        </tr>
                                    <?php endwhile; ?>
                                <?php else : ?>
                                    <tr>
